...
#define pinLED 6                            //status led
#define buttonset A0                          //button set
#define buttonnext A1                         //button next, +  
#define buttonprev A2                         //button pre, -
#define buttonback A3                         //button back, save, display setting
#define bulb 7                               // bulb relay 
#define seal 8                              // servo
#define motor_rev 4
#define motor_for 5
#define buzzer 12
...
unsigned long turner_dalay;
bool stateturner=true;
bool stateLED=true;
float tempC;
int humidity;
...
int read_capture_day_now;
int read_capture_month_now;
int read_capture_year_now;

int capture_day_now;
int capture_month_now;
int capture_year_now;
...
int counter_for_turner=0;
int turn_sec=0;
boolean turnmotor=false;
int min_turner=0;
int hr_turner=0;

int raw_data_turner=0;
int motor_stop_manual=0;

int date_set_=1;
String _date_set="day";

int date_set_day = 1;
int date_set_month = 1;
int date_set_year = 2020;
...
//  rtc.setDate(1, 8, 2021);   // Set the date to January 1st, 2014

//rtc.setDate(8, 17, 2021);
...
void loop() {
  ...
  if(display_screen == "date_now"){
    clearlcd();
    lcd.setCursor(0,0); lcd.print("Date Now dd.mm.yyyy");
...
if(display_screen == "manual motor"){
    if(display_settings){
      lcd.setCursor(0,0); lcd.print("  Manual  Motor       ");
      lcd.setCursor(0,1); lcd.print("  +CCW   -CW          ");
      ccw_cc_button();              
    }
}

if(display_screen == "set date"){
  lcd.setCursor(0,0); lcd.print("Set: dd/mm/yyyy   ");
  if(date_set_day < 10){
    lcd.setCursor(0,1); lcd.print(0);lcd.setCursor(1,1);lcd.print(date_set_day);
    }else{lcd.setCursor(0,1);lcd.print(date_set_day);}
  
  lcd.setCursor(2,1);lcd.print(".");

  if(date_set_month < 10){
    lcd.setCursor(3,1);lcd.print(0);lcd.setCursor(4,1);lcd.print(date_set_month);
    }else{lcd.setCursor(3,1);lcd.print(date_set_month);}

  lcd.setCursor(5,1);lcd.print(".");  

  lcd.setCursor(6,1);lcd.print(date_set_year);
          ...
          if(reading_set != buttonState_set){
            buttonState_set = reading_set;
              if(buttonState_set == HIGH){
                 date_set_day=date_set_day+1;
                  if(date_set_day==32){date_set_day=1;}
        ...
        if((millis()-lastDebounceTime) > debounceDelay){
              if(reading_next != buttonState_next){
                  ...
                  if(buttonState_next == HIGH){
                    date_set_month=date_set_month+1;
                    if(date_set_month==13){date_set_month=1;}   
          ...
          if(reading_back != buttonState_back){
            buttonState_back = reading_back;
              if(buttonState_back == HIGH){
               rtc.setDate(date_set_day, date_set_month, date_set_year);
                delay(1000);
                lcd.setCursor(0,1);lcd.print("Saving....      ");
                delay(3000);
                date_set_day=1;
                date_set_month=1;
                date_set_year=2020;
...
//                Serial.println("++ccw");
              ...
              if(!reading_next && motor_stop_manual == 1){
//                Serial.println("stop");
                offmotor();
                motor_stop_manual = 0;
                }
...
//                Serial.println("-cw");
              ...
              if(!reading_prev && motor_stop_manual == 2){
//                Serial.println("stop");
                offmotor();
                motor_stop_manual = 0;
                }        
    ...
    if(reading_back != buttonState_back){
      buttonState_back = reading_back;
        if(buttonState_back == HIGH){             
              ...
              if(menu==1){//Serial.println("Saving Temp");
              saving_yes_no();}
...
//  capture_sec_now = t.sec;
   ...
   capture_day_now = t.date;
   capture_month_now = t.mon;
   capture_year_now = t.year;


   read_capture_day_now = capture_day_now;
   read_capture_month_now = capture_month_now;
   read_capture_year_now = capture_year_now;

   EEPROM.write(8,capture_day_now);
   EEPROM.write(9,capture_month_now);
 //  EEPROM.write(11,capture_year_now);


  writeEepromInt(capture_year_now, 11);
...
void checkedTemperature(){
  ...
  if(tempC >= read_data_temp_high){
    bulb_status=0; 
    digitalWrite(bulb, HIGH);           // light On bulb
    } 
  else if(tempC <= read_data_temp_low){
    bulb_status=1; 
    digitalWrite(bulb, LOW);             //light Off Bulb
    
    }

  if(tempC >= 38){digitalWrite(buzzer, HIGH);}   
//  
...
jd_datecapture = JulianDate(read_capture_year_now, read_capture_month_now, read_capture_day_now);
jd_datenow = JulianDate(year_now, month_now, day_now);

days=jd_datenow - jd_datecapture;

//Serial.println("Day: " + String(days));
//Serial.print(String(read_capture_day_now) + "/");
//Serial.print(String(read_capture_month_now) + "/");
//Serial.println(read_capture_year_now);

if(days >= read_data_day_value ){digitalWrite(buzzer,HIGH);}else{digitalWrite(buzzer,LOW);}
...
raw_data_turner = (read_data_turner_hr * 3600) + (read_data_turner_min * 60);
Serial.println(counter_for_turner);
if(counter_for_turner == raw_data_turner){
    counter_for_turner = 0;
...
if(stateturner){
  if(turnmotor){
        if(micros() - turner_dalay > 1000000){     
        
        turn_sec = turn_sec + 1;
        Serial.println("turn motor");
        motor_forward();
        if (turn_sec == 5){Serial.println("Stop motor"); turnmotor=false;turn_sec=0;offmotor();stateturner = !stateturner;}
        turner_dalay=micros();
      }
      }
  }
...
read_capture_day_now = EEPROM.read(8);
read_capture_month_now = EEPROM.read(9);
//read_capture_year_now = EEPROM.read(11);

read_capture_year_now=readEepromInt(11);
...
void ledblinking(){ 
  if(micros() - led_dalay > 1000000){     
     stateLED = !stateLED;
     counter_for_turner = counter_for_turner + 1;
     led_dalay=micros();
    }
